...
public class TeamPeriodController {
    ...
    @Autowired
    public TeamPeriodController(TeamPeriodService teamPeriodService) {
        ...
        try {
            if(teamPeriod != null) {
                if(0 != teamPeriod.getTeamName().getId()) {
                    // Controleren Startdatum voor Einddatum
                    if (teamPeriod.getStartdate().before(teamPeriod.getEnddate())) {
                        teamPeriodService.save(teamPeriod);
                        return ResponseEntity.status(HttpStatus.OK).body("Teamperiode is succesvol toegevoegd!");
                    }else{
